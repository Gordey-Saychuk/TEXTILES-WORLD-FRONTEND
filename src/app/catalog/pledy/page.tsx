...
import { getPledsCatalog } from '../../lib/api/getTovars';
...
export default function Pledy() { 
  ...
  useEffect(() => {
    ...
    async function fetchData() { 
        try {
            const response = await getPledsCatalog(page, 8, category ? parseInt(category, 10) : undefined, sortId, filters);
    ...
    <div>
      <div className={styles.breadcrumbs}>
      	<Breadcrumbs
					paths={[
						{ name: 'Главная', href: '/' },
						{ name: 'Каталог', href: '/catalog/pledy' }  
					]}  
				/>   
        </div>    
      <div className={styles.page}>    
        <SliderCatalog title='Мягкие пледы' p='Каталог премиум пледов роскошного качества' img='/jpg/40815988aea8a6a5cd482d53075c3b42.jpg'  /> 
      </div>
